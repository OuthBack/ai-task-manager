...
export class TasksService {
  ...
  async create(createTaskDto: CreateTaskDto): Promise<Task> {
    this.logger.log(`Attempting to create a new task with title: "${createTaskDto.title}"`);
  ...
  async update(id: string, updateTaskDto: UpdateTaskDto): Promise<Task> {
    this.logger.log(`Attempting to update task with ID: "${id}" with data: ${JSON.stringify(updateTaskDto)}`);
    await this.findOne(id); // Verify task exists
    const updatedTask = await this.tasksRepository.update(id, updateTaskDto);
    if (!updatedTask) {
      this.logger.warn(`Task with ID: "${id}" not found during update operation.`);
      throw new NotFoundException('Task not found');
    }
  ...
  async remove(id: string): Promise<void> {
    ...
    if (!deleted) {
      this.logger.warn(`Task with ID: "${id}" not found during remove operation.`);
      throw new NotFoundException('Task not found');
    }
